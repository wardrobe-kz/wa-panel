...
export default function ContactChat({ params }: { params: { wa_id: string } }) {
  ...
  const [lastMessageReceivedAt, setLastMessageReceivedAt] = useState<
    Date | undefined
  >();
  const [contactRepository] = useState(() =>
    ContactBrowserFactory.getInstance()
  );
  const [supabase] = useState(() => createClient());
  const [messageTemplateSending, setMessageTemplateSending] =
      ...
      .on<Contact>(
        ...
        (payload) => {
          if (payload.new.last_message_received_at) {
            setLastMessageReceivedAt(
              new Date(payload.new.last_message_received_at)
            );
          }
        }
      )
              ...
              return (
                <div className="p-4 bg-white flex flex-row gap-4 items-center">
                  <span className="text-sm">
                    You can only send a message within 24 hours of the last
                    customer interaction. Please wait until the customer reaches
                    out to you again or send a template message.{" "}
                    <a
                  ...
                  <TemplateSelection onTemplateSubmit={onTemplateSubmit}>
                    <Button
                      disabled={messageTemplateSending}
                      className="min-w-fit"
                    >
